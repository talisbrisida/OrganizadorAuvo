    ...
    return (
        <div className="bg-white rounded-2xl shadow-xl shadow-stone-200/50 border border-stone-100 overflow-hidden">
                ...
                <thead className="bg-[#4d1c0c] text-white">
                    <tr className="uppercase text-[10px] tracking-widest font-semibold">
                        <th className="p-5 w-12 text-center">
                            <input type="checkbox" className="w-4 h-4 cursor-pointer accent-orange-500" checked={tarefasFiltradas.length > 0 && selecionados.length === tarefasFiltradas.length} onChange={toggleTodos} />
                        </th>
                    ...
                    {tarefasFiltradas.map(t => (
                        <tr key={t.id_tarefa} className={`hover:bg-orange-50/30 transition-colors duration-150 group ${selecionados.includes(t.id_tarefa) ? 'bg-orange-50/50' : ''}`}>
                            <td className="p-5 text-center">
                                <input type="checkbox" className="w-4 h-4 cursor-pointer accent-[#4d1c0c]" checked={selecionados.includes(t.id_tarefa)} onChange={() => toggleSelecionado(t.id_tarefa)} />
                            </td>
                            <td className="p-5">
                            ...
                            <td className="p-5">
                                <select className="w-full p-2.5 border border-stone-200 rounded-lg bg-stone-50 text-stone-700 text-xs focus:ring-2 focus:ring-[#4d1c0c]/20 outline-none cursor-pointer" value={t.cliente.zona_roteirizacao || ""} onChange={e => handleMudarZona(t.id_tarefa, t.cliente.nome, e.target.value)}>
                                    <option value="">Sem Zona</option>
                                    {zonas.map(z => <option key={z} value={z}>{z}</option>)}
                                </select>
                            </td>
                            <td className="p-5">
                                <select className="w-full p-2.5 border border-stone-200 rounded-lg bg-stone-50 text-stone-700 text-xs focus:ring-2 focus:ring-[#4d1c0c]/20 outline-none cursor-pointer" value={t.agendamento_atual.tecnico_alocado || ""} onChange={e => atualizar(t.id_tarefa, 'tecnico', e.target.value)}>
                            ...
                            <td className="p-5 text-center">
                                <button onClick={() => abrirModalAuvo(t)} className="p-2 bg-stone-100 hover:bg-[#4d1c0c] text-stone-600 hover:text-white rounded transition-colors border border-stone-200" title="Opções Auvo">⚙️</button>
                            </td>
                        </tr>
                    ))}
                </tbody>
            ...
            {tarefasFiltradas.length === 0 && (
                <div className="p-10 text-center text-stone-500 font-medium">Nenhum cliente encontrado com estes filtros.</div>
            )}
        </div>
    );
};
